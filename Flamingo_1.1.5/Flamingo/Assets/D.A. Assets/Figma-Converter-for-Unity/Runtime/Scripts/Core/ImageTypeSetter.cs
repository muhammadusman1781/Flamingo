...
namespace DA_Assets.FCU
{
    ...
    public class ImageTypeSetter : MonoBehaviourLinkerRuntime<FigmaConverterUnity>
    {
        ...
        public async Task SetInsideDownloadableFlag(FObject root)
        {
            await Task.Run(() =>
            {
                SetFlagDfs(root, insideAncestor: false);
            }, monoBeh.GetToken(TokenType.Import));
        }

        private void SetFlagDfs(FObject fobject, bool insideAncestor)
        {
            bool isNodeDownloadable = monoBeh.ImageTypeSetter.DownloadableIds.Contains(fobject.Id);
            bool isInside = insideAncestor || isNodeDownloadable;

            fobject.Data.InsideDownloadable = isInside;

            FcuLogger.Debug(
                $"IsInsideDownloadable | {isInside} | {fobject.Data.NameHierarchy}",
            ...
            {
                SetFlagDfs(child, insideAncestor: isInside);
            }
        }
